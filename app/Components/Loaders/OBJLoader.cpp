...
bool OBJLoader::fillScene(Scene *const scene,
    ...
    #pragma omp parallel shared(scene)
    {
        ...
        ::std::vector<Scene::TypeLights> lights {};
        ...
        for (::std::int32_t shapeIndex = 0; shapeIndex < shapesSize; ++shapeIndex) {
            ...
            for (::std::int32_t face = 0; face < faces; ++face) {
                ...
                for (::std::int32_t vertex = 0; vertex < faceVertices; vertex += 3) {
                    ...
                    if (materialId >= 0) {
                        ...
                        if (::MobileRT::hasPositiveValue(emission)) {
                            ...
                            {
                                lights.emplace_back(AreaLight {material, lambda(), triangle});
                            }
                        } else {
                            ...
                            auto itFoundMat {scene->materials_.cend()};
                            #pragma omp critical
                            {
                                itFoundMat  = ::std::find(scene->materials_.begin(), scene->materials_.end(), material);
                            }
                ...
                if (triangles.size() > 0 && triangles.size() % 10000 == 0) {
                    ...
                    const auto& lightPos {::boost::get<::Components::AreaLight>(lights.back()).getPosition()};
        ...
        ::std::vector<Scene::TypeLights> {}.swap(lights);
    }
