...
#include <vector>
#include <thread>
#include <future>  // Include for std::async/std::future
...
void BVH<T>::build(::std::vector<T> &&primitives) {
    ...
    // Parallel execution of the BVH construction
    auto parallelBuild = [&](size_t begin, size_t end) {
        // Similar code to existing build logic here
        do {
            const auto itCurrentBox {this->boxes_.begin() + currentBoxIndex};
            const ::std::int32_t boxPrimitivesSize {endBoxIndex - beginBoxIndex};
            const auto itBegin {buildNodes.begin() + begin};
            const auto itEnd {buildNodes.begin() + end};
    ...
    size_t partitionSize = primitivesSize / numThreads;
    
    for (size_t i = 0; i < numThreads; ++i) {
        size_t begin = i * partitionSize;
        size_t end = (i == numThreads - 1) ? primitivesSize : begin + partitionSize;
        futures.push_back(std::async(std::launch::async, parallelBuild, begin, end));
    }
