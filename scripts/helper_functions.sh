...
addCommandToPath() {
  ...
  visualStudioVersion=$(ls -la "/c/Program Files/Microsoft Visual Studio/" | grep ".*[[:digit:]]$" | rev | cut -d ' ' -f1 | rev | tail -1 || true);
  echo "Detected Visual Studio version: ${visualStudioVersion}";
  msvcVersion=$(ls -la "/c/Program Files/Microsoft Visual Studio/${visualStudioVersion}/Enterprise/VC/Tools/MSVC/" | grep "[[:digit:]]\.[[:digit:]].*" | rev | cut -d ' ' -f1 | rev | tail -1 || true);
